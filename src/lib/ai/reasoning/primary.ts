...
export class PrimaryReasoningProvider implements ReasoningProvider {
  ...
  constructor() {
    this.apiKey = process.env.REASONING_PRIMARY_API_KEY || process.env.GEMINI_API_KEY || '';
    if (!this.apiKey) {
      console.warn('REASONING_PRIMARY_API_KEY/GEMINI_API_KEY not set, Primary provider will fail');
    }
  }
  ...
  async complete(prompt: string, options?: { systemPrompt?: string; temperature?: number; maxTokens?: number }): Promise<ReasoningResult> {
    ...
    if (!this.apiKey) {
      throw new Error('Primary reasoning API key not configured');
    }
    ...
    if (!response.ok) {
      ...
      throw new Error(`Primary (Gemini) reasoning failed: ${response.status} - ${error}`);
    }
